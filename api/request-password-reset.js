...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Attempt initialization with current credentials; if parsing fails, try fallback
    let credential = null;
    try {
      credential = cert({ projectId, clientEmail, privateKey });
    } catch (certErr) {
      console.warn('[Password Reset] Primary key parse failed, trying fallback:', certErr.message);
      try {
