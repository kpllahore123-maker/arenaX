                    ...
                    {transactions.map((tx) => (
                      <div key={tx.id} className="p-3 bg-[#171b2e] border border-[#252a45] rounded-xl flex items-center justify-between gap-3 text-xs">
                        <div className="flex items-center gap-2.5">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs ${tx.type === 'deposit' ? 'bg-[#3ddc84]/10 text-[#3ddc84]' : 'bg-[#e8404a]/10 text-[#e8404a]'}`}>
                            <i className={`fas ${tx.type === 'deposit' ? 'fa-arrow-down' : 'fa-arrow-up'}`}></i>
                          </div>
                          <div>
                            <div className="font-semibold text-white uppercase">{tx.type}</div>
                            <div className="text-[10px] text-[#4a5070]">{tx.timestamp}</div>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className={`font-sans font-bold text-sm ${tx.type === 'deposit' ? 'text-[#3ddc84]' : 'text-[#e8404a]'}`}>
                            {tx.type === 'deposit' ? '+' : '-'}{tx.amount} AX
                          </div>
                          <div className="text-[9px] uppercase tracking-wider text-[#8890b0] font-medium">{tx.account}</div>
                        </div>
                      </div>
                    ))}
                  </div>
